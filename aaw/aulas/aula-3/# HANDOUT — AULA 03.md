...
usar POST em vez de GET; rota não representando recurso alunos; retornar 12 mil alunos de uma vez.

2. Seu redesenho (método + rota + status codes):
GET/api/alunos
Status: 
200 OK alunos encontrados
...
1. Qual(is) problema(s) de design vocês identificam?
uso de GET para apagar aluno.
não usar DELETE.
deletarAluno descrevendo ação não recurso.
não retornar 200 OK se aluno não existir
...
1. Qual(is) problema(s) de design vocês identificam?
status 200 OK: usar 201 Created.
API retornar aluno criado, ID gerado, não buscar na lista inteira.
...
1. Qual(is) problema(s) de design vocês identificam?
URL muito longa, complexa.
IDs excessivos até disciplina.
matricula não já identificar contexto.
...
1. Qual(is) problema(s) de design vocês identificam?
API retorna 200 OK em erro;
Retornar HTML e API trabalha com JSON;
Código HTTP não informa aluno não encontrado;
...
1. Qual(is) problema(s) de design vocês identificam?
Rota  não representar nota/disciplina do aluno;
dados não estão no body de requisição;
dados na URL;
atualizarNotaParcial não representa recurso.
